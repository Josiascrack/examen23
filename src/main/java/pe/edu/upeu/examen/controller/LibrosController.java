...
public class LibrosController {
	...
	public ResponseEntity<Libros> save(@RequestBody Libros lib){
		try {
			Libros li = ls.create(new Libros(lib.getId(),lib.getTitulo(), lib.getPaginas(), lib.getDescripcion(), lib.getAutor(), lib.getEditorial()));
	...
	public ResponseEntity<Libros> update(@RequestBody Libros li, @PathVariable("id") int id){
		try {
			...
			if (ul.getId()>0) {
				...
				return new ResponseEntity<>(ls.create(ul),HttpStatus.OK);
			} else {
				return new ResponseEntity<>(HttpStatus.NOT_FOUND);
			}
		} catch (Exception e) {
